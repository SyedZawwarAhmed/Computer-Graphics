...
void myDisplay(void)
{
	//drawPolyLineFile((char*)("birdhead.dat"));		   // send all output to display
	//drawPolyLineFile((char*)("house.dat"));
	int rows = 5;
	int columns = 5;

	// Calculate the spacing between shapes in the grid
	float spacingX = 2.0;
	float spacingY = 2.0;

	// Loop to draw the shapes in the tiling grid
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < columns; j++) {
			glPushMatrix();
			glTranslatef(j * spacingX, i * spacingY, 0.0);
